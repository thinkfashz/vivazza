...
// Access the global Leaflet variable from the script tag in index.html
declare const L: any;
...
// Default to Talca Plaza de Armas
...
const LocationMap: React.FC<LocationMapProps> = ({ isOpen, onClose, onConfirm, initialCoords }) => {
  ...
  useEffect(() => {
    ...
    // Small delay to ensure container is rendered dimensions
    setTimeout(() => {
        ...
        // Initial fetch if we have coords
        if (initialCoords) {
  ...
  const fetchAddress = async (lat: number, lng: number) => {
    ...
    try {
      ...
      if (data && data.display_name) {
        // Clean up address a bit (take first 3 parts)
      ...
      setCurrentAddress('Ubicación seleccionada (Dirección no encontrada)');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in-up">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="relative w-full h-[400px] bg-gray-100">
            ...
            {/* Loading Overlay */}
            {isLoading && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/90 px-4 py-1 rounded-full text-xs font-bold shadow-md z-[500] animate-pulse">
                    Buscando dirección...
                </div>
            )}
        </div>
